---
title: "Econometrics 2 capstone final report data construction code"
...
UNICEF defines a household's access to water as "basic" if it satisfies the following conditions:
 ...
 UNICEF actually has tiered definitions for water, where we want to capture dummies for all of them as one of ou extensions. They are defined as:
 ...
 - Limited: The same as basic except travel time exceeds 30 minutes.
 
 - Unimproved: drinking from an unprotected dug well or unprotected spring.
...
s12ai <- s12ai %>% 
  ...
  mutate(
    ...
    unimproved_dummy = case_when(
      
     drinking_source %in% c(11, # Unprotected well
                            13) #spring
       ~ 1,
       TRUE ~ 0 
      )) %>% 
...
data <- s10ai %>% 
  ...
  select(id, depression_dummy, basic_access_dummy, sex_dummy, age, not_christian_dummy, rural_dummy, drinking_source_distance_mins, safely_managed_dummy, limited_dummy, unimproved_dummy, no_service_dummy) #getting data columns into a helpful order
