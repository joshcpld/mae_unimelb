...
* I then specify and estimate using OLS any deterministic trend and seasonality components I believe to exist:
  ...
  + This means the residuals of the linear model, or the detrended series, also represents the cyclical component of our time series.
...
* Now we are ready to conduct the Augmented Dicky-Fuller (ADF) test:
  ...
  + I first set the maximum number of potential lags for our ADF test using the Ng & Perron (1995) testing procedure. This is equal to 12 lags for our data.
...
kmax <- ceiling(12*(length(data$detrended)/100)^1/4)
...
for (i in 1:kmax-1){
  ...
  df.data = ur.df(data$detrended, type = "drift", lags = k)

  tstat = df.data@testreg$coefficients[k+2,3]
