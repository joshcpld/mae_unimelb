...
trend_model <- lm(value ~ time, data = data)

summary(trend_model)

trend_model_data <- tibble(
  date       = data$date,
  value      = data$value,
  fitted     = fitted(trend_model),
  residuals  = resid(trend_model)
)

c1 <- ggplot(trend_model_data, aes(x = date, y = value)) +
  geom_line() +
  geom_line(aes(y = fitted), color = "red", linewidth = 1) +
  labs(
    title = "Chart 1: Time trend \n model fitted values",
    y = "value",
    x = "date"
  ) 

c2 <- ggplot(trend_model_data, aes(x = date, y = residuals)) +
  geom_line() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    title = "Chart 2: Time trend \n model residuals",
    y = "residuals",
    x = "date"
  ) +
...
ar_summary <- bind_rows(
  select_ar_summary(trend, data),
  select_ar_summary(one_break, data),
  select_ar_summary(two_break, data)
) %>%
  mutate(model = c("trend", "one break", "two breaks"))
...
ggplot(Z_data, aes(x = date, y = residual)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    title = "Residuals (Z_t) from trend, one-break, and two-break models",
    x = "date",
    y = "residual"
  )
...
adf_table <- bind_rows(
  get_adf(residuals(trend_model), "trend"),
...
pval_tbl <- tibble(
  model   = c("trend", "one break", "two breaks"),
  resid   = list(
