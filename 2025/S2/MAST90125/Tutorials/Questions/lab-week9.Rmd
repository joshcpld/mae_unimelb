---
title: 'Week 9 Lab -- MAST90125: Bayesian Statistical learning'
header-includes:
   - \usepackage{bm}
   - \usepackage{amsmath}
output: 
...
In this lab, we will continue discussing how to write code for Gibbs sampling of linear models with proper priors. We will use the data in \texttt{USJudgeRatings.csv}, which is available on Canvas. We will assume the variable \texttt{RTEN} is the response and the other variables are predictors. Meanwhile, how to analyze the generated chains will be included. In addition, we will show another example to see the difference between empirical Bayes and full Bayes.
...
\begin{itemize}
\item Linear mixed model/ ridge regression (flat prior for $\bm \beta_0$, $p(\tau_e) = \text{Ga}(\alpha_e,\gamma_e)$ with $\tau_e = (\sigma_e^2)^{-1}$, and $\bm \beta \sim \mathcal{N}({\bf 0},\sigma^2_\beta{\bf I})$ with $(\sigma^2_\beta)^{-1} = \tau_\beta \sim  \text{Ga}(\alpha_\beta,\gamma_\beta)$.)  
\end{itemize}
