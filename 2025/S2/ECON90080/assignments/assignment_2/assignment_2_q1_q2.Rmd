...
# 2) Convert to quarterly (monthly → quarterly mean), then log-diff per series
g_long <- raw_long %>%
...
xcor_mat <- g_wide %>%
  ...
  mutate(
    lag_0 = cons_g,
    lag_1 = dplyr::lag(cons_g, 1),
    lag_2 = dplyr::lag(cons_g, 2),
    lag_3 = dplyr::lag(cons_g, 3)
  ) %>%
  summarise(
    across(starts_with("lag_"),
           ~ round(cor(.x, gdp_g, use = "complete.obs"), 2)),
    .groups = "drop"
  )
